							...
							<c:forEach var="shop" items="${shops}">
								<tr>
									...
									<td>
										<c:choose>
											<c:when test="${not empty shop.description}">
												<c:out value="${fn:substring(shop.description, 0, 50)}${fn:length(shop.description) > 50 ? '...' : ''}"/>
											</c:when>
											<c:otherwise>
									...
									<td class="table__actions">
										...
										<c:choose>
											<c:when test="${shop.status == 'Active'}">
												<form method="post" action="${cPath}/seller/shops/hide" style="display:inline;">
													<input type="hidden" name="id" value="${shop.id}" />
													<button type="submit" class="button button--ghost">Ẩn</button>
												</form>
											</c:when>
											<c:otherwise>
												<form method="post" action="${cPath}/seller/shops/restore" style="display:inline;">
													<input type="hidden" name="id" value="${shop.id}" />
